...
subtitle: transformer의 전역 문맥은 유지하되 부족한 locality와 느슨한 decoder aggregation을 보완하기 위해, 이웃 token 관계를 강화하고 인접 단계 특징을 점진적으로 축소·집약한
  FSPNet.
summary: FSPNet은 NL-TEM으로 인접 token의 고차 관계를 강화하고, AIM을 포함한 Feature Shrinkage Decoder가 이웃 transformer feature를 층별로 모은다.
description: FSPNet은 NL-TEM으로 인접 token의 고차 관계를 강화하고, AIM을 포함한 Feature Shrinkage Decoder가 이웃 transformer feature를 층별로 모은다.
...
venue: CVPR 2023
paper_year: 2023
paper_authors: Zhou Huang, Hang Dai, Tian-Zhu Xiang, Shuo Wang, Huai-Xin Chen, Jie Qin, Huan Xiong
...
code_url: https://github.com/ZhouHuang23/FSPNet
takeaways:
